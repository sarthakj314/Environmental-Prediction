...
def log_model_params(model, name = ''):
        ...
        for i, (x, GT, dates) in tqdm(enumerate(trainloader), total=num_iters, leave=False):
            ...
            for i in range(config['num_D']):
                for j in range(len(pred_fake[i]) - 1):
                    loss_G_FM += D_weights * feat_weights * criterion_FM(pred_fake[i][j], pred_real[i][j].detach())
...
def main():
    ...
    log_model_params(generator, name='Generator')
    log_model_params(discriminator, name='Discriminator')

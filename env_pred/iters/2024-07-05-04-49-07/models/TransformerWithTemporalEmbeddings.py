...
class TransformerWithEmbeddings(nn.Module):
    ...
    def forward(self, 
        ...
        self.temporal_emb = self.temporal_embeddings(temporal_input)
        # Zero out any temporal_emb for which src_key_padding_mask is False
        if src_key_padding_mask is not None:
            self.temporal_emb = self.temporal_emb * src_key_padding_mask.unsqueeze(-1).float()
        src = src + self.positional_embeddings(pos_indices) + self.temporal_emb

...
class SentinelDataset():
    ...
    def getitem(self, idx, return_image=True):
        return self.get_image(idx) if return_image else None, self.get_date(idx), self.get_coords(idx)
    
    def date_to_idx(self, date):
        return self.DATES_SHORT_DICT[date[:7]]
...
class Pix2PixHD_Dataset(Dataset):
    def __init__(self, valid_indexes_path, data_directory, dates_path, month_dist, data_cap = None):
        self.tmp_indexes = np.load(valid_indexes_path)
        self.sent2 = SentinelDataset(self.tmp_indexes, data_directory, dates_path)
        self.month_dist = month_dist
        self.indexes = []
    ...
    def __getitem__(self, idx):
        ...
        GT, date, coords = self.sent2.getitem(idx)
        GT = self.process(GT)
        past_idx = self.sent2.get_past_month(date, coords, self.month_dist)
        if past_idx == -1:
            return None
...
if __name__ == '__main__':
    ...
    trainset = Pix2PixHD_Dataset(config['train_data_path'], config['data_directory'], config['dates_path'], data_cap = None)
    ...
    print('Total Validation Data:', len(valset))
...
from AttentionBasedFusion import TemporalAttention
...
TemporalAttention: (B, transformer_dim) & T*intermediate_outputs -> intermediate_outputs
...
class FullModel(nn.Module):
    def __init__(self, 
                 ...
                 transformer_dim=512,
                 block_sizes_encoder=[2, 2, 2, 1],
                 block_sizes_decoder=[2, 2, 2, 2, 2]
                 ):
        super().__init__()
        ...
        self.attention = nn.ModuleList(
            [TemporalAttention(transformer_dim, channel_dim) for channel_dim in self.encoder.channel_sizes]
        )
        self.decoder = ResidualDecoder(block_sizes=block_sizes_decoder)
        
    def forward(self, x, temporal_input):
        ...
        transformer_output = self.transformer(encoder_output, temporal_input)
        ...
        for i, attended_output in enumerate(attended_outputs):
            print(f"Attended output {i} shape:", attended_output.shape)
...
temporal_input = torch.randint(0, 37, (10, 12)).to(device)
tgt_month = torch.randint(0, 37, (10, 1)).to(device)
output = model(input_tensor, temporal_input, tgt_month)
